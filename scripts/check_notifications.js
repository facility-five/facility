const { Client } = require('pg');

const sqls = {
  functions: `SELECT n.nspname AS schema_name, p.proname AS function_name, pg_get_functiondef(p.oid) AS definition
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE p.proname IN ('create_notification','notify_task_insert','notify_task_status_change')
ORDER BY function_name;`,

  triggers: `SELECT tg.tgname AS trigger_name,
       c.relname AS table_name,
       pg_get_triggerdef(tg.oid) AS definition
FROM pg_trigger tg
JOIN pg_class c ON tg.tgrelid = c.oid
WHERE c.relname = 'admin_tasks'
  AND tg.tgname IN ('trg_admin_tasks_notify_insert','trg_admin_tasks_notify_status');`,

  table_exists: `SELECT table_schema, table_name
FROM information_schema.tables
WHERE table_name = 'notifications';`,

  policies: `SELECT schemaname, tablename, policyname, permissive, roles, qual AS using_qual, with_check AS with_check_qual
FROM pg_policies
WHERE tablename = 'notifications';`,

  notifications_for_email: `WITH profile AS (
  SELECT id, email FROM public.profiles WHERE email = 'wfss1982@gmail.com' LIMIT 1
)
SELECT n.id, n.user_id, p.email, n.title, n.message, n.is_read, n.created_at
FROM public.notifications n
JOIN profile p ON p.id = n.user_id
ORDER BY n.created_at DESC
LIMIT 50;`,

  recent_admin_tasks: `SELECT id, title, created_by, assigned_to, status, created_at
FROM public.admin_tasks
ORDER BY created_at DESC
LIMIT 25;`
};

async function run() {
  const dbUrl = process.env.DB_URL;
  if (!dbUrl) {
    console.error('Missing DB_URL env var. Set DB_URL to the Postgres connection string.');
    process.exit(2);
  }

  const client = new Client({ connectionString: dbUrl });
  await client.connect();

  const out = {};
  try {
    for (const [k, q] of Object.entries(sqls)) {
      try {
        const res = await client.query(q);
        out[k] = { rowCount: res.rowCount, rows: res.rows };
      } catch (err) {
        out[k] = { error: err.message };
      }
    }
  } finally {
    await client.end();
  }

  console.log(JSON.stringify(out, null, 2));
}

run().catch(err => {
  console.error('Unhandled error:', err);
  process.exit(1);
});
